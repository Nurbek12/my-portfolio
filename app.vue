<template>
  <div :class="{'dark':dark}">
    <app-navbar v-model="dark" />
  
    <header class="h-screen w-full relative dark:bg-background-900 bg-white">
      <div class="relative z-10 container py-20 h-full items-center grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="relative space-y-4 z-10">
          <h1 class="text-4xl font-medium dark:text-white">
            <span class="text-primary-600">Нурбек Рустамов</span>, Фул Стек разработчик</h1>
          <p class="dark:text-gray-200 font-light">Привет! Я Нурбек Рустамов, разработчик с опытом 2-3x лет работы в различных проектах. Я специализируюсь на создании высокопроизводительных и масштабируемых веб-приложений. Мои навыки включают в себя фронтенд, бэкенд и десктоп разработку, а также оптимизацию производительности и SEO</p>
    ...
    <main class="min-h-screen w-full dark:bg-background-900 bg-white">
  
      <section class="container pt-32 pb-10 h-full relative z-10" id="about">
        <div class="text-center">
          <h1 class="dark:text-white text-2xl">Обо мне - Биография</h1>
        </div>
      ...
      <section class="container pt-32 pb-10 h-full" id="projects">
        <div class="text-center">
          <h1 class="dark:text-white text-2xl">Мои проекты</h1>
        </div>
      ...
      <section class="container pt-32 pb-10 h-full" id="skills">
        <div class="text-center">
          <h1 class="dark:text-white text-2xl">Мои навыки</h1>
        </div>
  
        <div class="mt-12 grid grid-cols-3 gap-4">
          
          <div v-for="s,i in skills" :key="i">
            <h3 class="dark:text-white font-medium"  :class="['text-start md:text-center','text-center','text-end md:text-center'][i]" >{{ s.title }}</h3>
            ...
            <div class="mt-6 flex flex-wrap items-center gap-4" :class="['justify-start','justify-center','justify-end'][i]">
              ...
              <div v-for="p,j in s.programs" :key="j" class="text-center space-y-2">
                <div class="w-[80px] h-[80px] rounded dark:bg-[#1f1f23] dark:border-0 border flex justify-center items-center p-4">
                  <img :src="p.icon" class="w-full h-full object-contain">
                </div>
                <p class="dark:text-background-200 text-sm font-medium">{{ p.title }}</p>
              </div>
      ...
      <section class="container pt-32 pb-10 h-full" id="contacts">
        <div class="text-center">
          <h1 class="dark:text-white text-2xl">Контакты</h1>
        </div>
  
        <div class="mt-6 py-2 rounded-xl grid grid-cols-1 md:grid-cols-2 gap-8">
          <div class="rounded-lg p-8 dark:bg-background-800 bg-gray-200 flex flex-col justify-between gap-4">
            <div v-for="link,i in socialLinks" :key="i" class="flex items-center gap-8">
              <div>
                <component :is="link.icon" class="dark:text-white size-8" />
              </div>
              <div>
                <h2 class="dark:text-white font-medium">{{ link.title }}</h2>
                <a :href="link.url" class="dark:text-gray-300 hover:text-primary-700 text-sm hover:underline">{{ link.name }}</a>
              </div>
            </div>
          </div>
          ...
          <form @submit.prevent="handlePost" class="w-full bg-background-00 pt-6 rounded-lg space-y-4 pb-20 md:pb-0">
            <div>
              <h1 class="dark:text-white text-lg">Свяжитесь со мной</h1>
...
const dark = ref(JSON.parse(localStorage.getItem('color-theme') || 'true'))
